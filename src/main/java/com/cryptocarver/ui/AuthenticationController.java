...
public class AuthenticationController {
     ...
     */
    public void loadSecureMessagingProfile(com.cryptocarver.model.payments.PaymentProfile profile) {
        if (profile == null) {
            throw new IllegalArgumentException("Laboratory profile is required");
        }
        if (profile.getType() != com.cryptocarver.model.payments.PaymentProfile.ProfileType.SECURE_MESSAGING) {
        ...
        if (macHsmKeyCombo != null) {
            ...
            try {
                if (isPkcs11MacSource()) {
                    ...
                    macHsmKeyCombo.getItems().addAll(com.cryptocarver.crypto.hsm.SimulatedHsmProvider.getInstance().listKeyIds(
    ...
    public void saveCurrentKeyToHsm() {
        try {
            ...
            com.cryptocarver.crypto.hsm.KeyMaterial km = com.cryptocarver.crypto.hsm.KeyMaterialFactory.fromSecretKey(
                ...
                java.util.Set.of(com.cryptocarver.crypto.hsm.KeyUsage.MAC)
            );
