...
public class SymmetricCipher {
    ...
    private static SecretKey createSecretKey(byte[] key, String algorithm) {
        String algo = normalizeAlgorithm(algorithm);
        return new SecretKeySpec(normalizeKeyMaterial(key, algorithm), algo);
    }

    /**
     * Expands a two-key 3DES key (K1 || K2) to the JCE three-component form
     * (K1 || K2 || K1). Other valid key material is returned unchanged.
     */
    static byte[] normalizeKeyMaterial(byte[] key, String algorithm) {
        if (isTripleDes(algorithm) && key.length == 16) {
            byte[] expandedKey = Arrays.copyOf(key, 24);
    ...
    private static boolean isTripleDes(String algorithm) {
        return algorithm != null && (algorithm.contains("3DES") || algorithm.contains("Triple"));
    }
    ...
    private static void validateKeyLength(byte[] key, String algorithm) {
        ...
        } else if (isTripleDes(algorithm)) {
            if (key.length == 16 || key.length == 24) {
                return;
            }
            throw new IllegalArgumentException(
                    String.format("%s requires a 16- or 24-byte (128- or 192-bit) key, got %d bytes",
    ...
    /**
     * Returns the IV/nonce length generated by the UI for a transformation.
     * A zero result means that the selected mode does not use an IV.
     */
    public static int getRecommendedIvLength(String algorithm, String mode) {
        ...
        if ("ChaCha20".equals(algorithm) || "ChaCha20-Poly1305".equals(algorithm)) {
            return 12;
        }
