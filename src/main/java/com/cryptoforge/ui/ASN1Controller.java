...
public class ASN1Controller {
    ...
    private void handleParseASN1() {
        try {
            ...
            if (reporter != null) {
                com.cryptoforge.model.OperationResult result = com.cryptoforge.model.OperationResult.forOperation("ASN.1 Parse")
                        .input(data)
                        .details(histDetails)
    ...
    private void handleEncodeASN1() {
        try {
            ...
            if (encoded != null) {
                ...
                if (reporter != null) {
                    com.cryptoforge.model.OperationResult result = com.cryptoforge.model.OperationResult.forOperation("ASN.1 Encode")
                            .output(encoded)
                            .details(histDetails)
    ...
    private void handleEditASN1Node(ASN1TreeNode node) {
        ...
        dialog.showAndWait().ifPresent(hex -> {
            try {
                ...
                byte[] reencoded = com.cryptoforge.asn1.ASN1Editor.editNodeAndReencode(rootNode, node, newBytes);
        ...
        dialog.showAndWait().ifPresent(format -> {
            try {
                String result = format.equals("JSON")
                    ? com.cryptoforge.asn1.ASN1TreeExporter.toJson(node)
                    : com.cryptoforge.asn1.ASN1TreeExporter.toMarkdown(node);
