...
import com.cryptoforge.model.ClipboardShelfManager;
import com.cryptoforge.model.AppSettings;
import com.cryptoforge.service.KeyCertificateFormatService;
import com.cryptoforge.service.KeyCertificateFormatService.DetectionResult;
        ...
        if (file != null) {
            ...
            try {
                ...
                if (str.contains("BEGIN") || str.trim().startsWith("{") || str.trim().startsWith("ssh-")) {
                    ...
                    workbenchInputArea.setText(com.cryptoforge.util.DataConverter.bytesToHex(content));
                }
                handleParse(null);
            } catch (Exception e) {
                if (statusReporter != null) {
    ...
    private byte[] getRawBytes(String input) {
        if (input.contains("BEGIN") || input.startsWith("{") || input.startsWith("ssh-")) {
            ...
            if (input.matches("^[0-9a-fA-F\\s]+$")) {
                return com.cryptoforge.util.DataConverter.hexToBytes(input.replaceAll("\\s", ""));
            } else {
    ...
    private void publishResult(String operationSuffix, String output) {
        if (statusReporter != null) {
            ...
            if (lastParsedResult != null && lastParsedResult.hasPrivateKey) {
                if (AppSettings.getInstance().getSecretVisibility() == com.cryptoforge.model.SecretVisibility.REDACTED) {
                    inputStr = "*** REDACTED ***";
                    if (!operationSuffix.contains("Summary") && !operationSuffix.contains("Validate")) {
                        output = "*** REDACTED ***";
                    }
                } else if (AppSettings.getInstance().getSecretVisibility() == com.cryptoforge.model.SecretVisibility.MASKED) {
                    inputStr = "*** MASKED ***";
                    if (!operationSuffix.contains("Summary") && !operationSuffix.contains("Validate")) {
                        output = "*** MASKED ***";
                    }
