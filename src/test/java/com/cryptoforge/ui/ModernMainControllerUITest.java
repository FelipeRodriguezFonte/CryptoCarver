...
class ModernMainControllerUITest {
    ...
    void testEveryRegisteredNavigationPathAvoidsThePlaceholder() throws Exception {
        ...
        runAndWait(() -> {
            try {
                ...
                for (com.cryptoforge.model.OperationDescriptor operation
                        : com.cryptoforge.model.OperationRegistry.getInstance().getAll()) {
    ...
    void testClearInvalidatesExpandedResultSnapshot() throws Exception {
        ...
        runAndWait(() -> {
            try {
                ...
                controller.publish(com.cryptoforge.model.OperationResult.forOperation("Snapshot test")
    ...
    void testPublishedInputAndOutputAreStoredAsSensitiveHistoryDetails() throws Exception {
        ...
        runAndWait(() -> {
            ...
            controller.publish(com.cryptoforge.model.OperationResult.forOperation("SHA-256")
                    .input("hash input".getBytes(java.nio.charset.StandardCharsets.UTF_8))
                    .output(new byte[] {0x01, 0x23, 0x45})
                    .detail("Algorithm", "SHA-256")
                    .build());

            com.cryptoforge.model.HistoryItem item = controller.getHistoryManager().getHistoryItems().get(0);
            assertTrue(item.getStructuredDetails().stream().anyMatch(detail ->
                    detail.name().startsWith("Input")
                            && "hash input".equals(detail.value())
                            && detail.classification() == com.cryptoforge.model.OperationDetail.Classification.SENSITIVE));
            assertTrue(item.getStructuredDetails().stream().anyMatch(detail ->
                    detail.name().startsWith("Output")
                            && "012345".equals(detail.value())
                            && detail.classification() == com.cryptoforge.model.OperationDetail.Classification.SENSITIVE));

            String masked = com.cryptoforge.utils.HistoryReportExporter.toMarkdown(
                    item, com.cryptoforge.model.SecretVisibility.MASKED);
            assertFalse(masked.contains("hash input"));
            assertTrue(masked.contains("***MASKED***"));
            String unsafe = com.cryptoforge.utils.HistoryReportExporter.toMarkdown(
                    item, com.cryptoforge.model.SecretVisibility.FULL_LAB);
        ...
        runAndWait(() -> {
            try {
                ...
                controller.addToHistory("Hashing", java.util.List.of(
                        com.cryptoforge.model.OperationDetail.publicDetail("Result", "OK"),
                        com.cryptoforge.model.OperationDetail.secretDetail("Laboratory key", "all-visible-in-lab")));
                controller.addToHistory("File Cipher (Streaming)", java.util.List.of(
                        com.cryptoforge.model.OperationDetail.publicDetail("Algorithm", "SHA-256")));
                historyController.refresh();
                javafx.scene.control.TableView<?> table = getField(historyController, "historyTable");
                assertEquals(2, table.getItems().size());
                assertTrue(table.getItems().stream()
                        .map(com.cryptoforge.model.HistoryItem.class::cast)
                ...
                assertEquals("Hashing", ((com.cryptoforge.model.HistoryItem) table.getItems().get(0)).getOperation());
                ...
                assertEquals("File Cipher (Streaming)", ((com.cryptoforge.model.HistoryItem) table.getItems().get(0)).getOperation());
                moduleFilter.setValue("Generic");
                table.getSelectionModel().selectFirst();
                javafx.scene.control.TableView<com.cryptoforge.model.OperationDetail> details = getField(historyController, "detailsTable");
                javafx.scene.control.ComboBox<com.cryptoforge.model.SecretVisibility> visibility = getField(historyController, "visibilityCombo");
                visibility.setValue(com.cryptoforge.model.SecretVisibility.FULL_LAB);
                ...
                visibility.setValue(com.cryptoforge.model.SecretVisibility.MASKED);
                visibility.getOnAction().handle(new javafx.event.ActionEvent());
    ...
    void testExpandedResultUsesLatestPublishedOperation() throws Exception {
        ...
        runAndWait(() -> {
            try {
                controller.publish(com.cryptoforge.model.OperationResult.forOperation("DES Key Generation")
                        .output("DES-RESULT".getBytes(java.nio.charset.StandardCharsets.UTF_8)).build());
                controller.publish(com.cryptoforge.model.OperationResult.forOperation("EdDSA Key Generation")
                ...
                com.cryptoforge.model.SecretVisibility originalVisibility = com.cryptoforge.model.AppSettings.getInstance()
                        .getSecretVisibility();
                try {
                    com.cryptoforge.model.AppSettings.getInstance()
                            .setSecretVisibility(com.cryptoforge.model.SecretVisibility.REDACTED);
                    controller.publish(com.cryptoforge.model.OperationResult.forOperation("Signature Verification")
    ...
    void testBatchRunnerFunctional() throws Exception {
        ...
        runAndWait(() -> {
            try {
                ...
                com.cryptoforge.model.HistoryManager hm = getField(controller, "historyManager");
        ...
        runAndWait(() -> {
            try {
                ...
                com.cryptoforge.model.HistoryManager hm = getField(mainControllerRef.get(), "historyManager");
                assertNotNull(hm, "HistoryManager should be initialized");
                assertFalse(hm.getHistoryItems().isEmpty(), "History should not be empty");
                com.cryptoforge.model.HistoryItem item = hm.getHistoryItems().get(hm.getHistoryItems().size() - 1);
    ...
    void testGenericModulePublishing() throws Exception {
        ...
        runAndWait(() -> {
            try {
                com.cryptoforge.model.HistoryManager hm = getField(controllerRef.get(), "historyManager");
                assertNotNull(hm, "HistoryManager should be initialized");
                assertFalse(hm.getHistoryItems().isEmpty(), "History should not be empty");
