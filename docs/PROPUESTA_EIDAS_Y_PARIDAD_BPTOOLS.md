...
| Carril pagos — Thales Key Block | **Parcial, y a propósito**: `ThalesKeyBlockOperations` lee y construye las partes en claro — cabecera de 16 caracteres, la tabla de usos completa con los algoritmos y modos que cada uso permite, y los bloques de cabecera opcionales con sus reglas. Dos nodos y sección propia en Claves. **No cifra ni descifra los datos de clave**: el capítulo 8 da los algoritmos (3DES-CBC con los bytes 0-7 de la cabecera como IV y CBC-MAC de 4 bytes; AES-CBC con los bytes 0-15 y CMAC de 8) pero **nunca dice cómo salen de la LMK las claves de cifrado y de MAC** — las cláusulas 8.6 y 8.7 dicen sólo «una variante de la LMK». Un solo key block real de un payShield, con su LMK, lo cierra en una tarde. |
