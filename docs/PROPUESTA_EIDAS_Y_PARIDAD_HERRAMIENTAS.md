...
| 8 | **HCE y tokenización**: Visa LUK/MSD/qVSDC, Mastercard CVC3/PIN-CVC3 y DS, ICC Dynamic Number, CAP y AMEX CSC | **Parcial**: `VisaHceOperations` (`VisaHceOperationsTest`) cubre LUK, MSD y qVSDC; `MastercardDataStorage` (`MastercardDataStorageTest`) cubre DSPK, Digest y Summary. CVC3 y PIN-CVC3 permanecen aparcados porque la herramienta externa ignora UN y ATC. El intento del 26-09-2026 de capturar ICC Dynamic Number, CAP y AMEX CSC quedó bloqueado por el control de teclado de la máquina virtual; véase `docs/CAPTURAS_CALCULADORA_HCE.md` §6. No hay clase ni test nuevos para estas operaciones porque no se obtuvo ningún vector. |
