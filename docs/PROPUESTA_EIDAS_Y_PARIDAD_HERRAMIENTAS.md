...
Revisado el 26 de septiembre de 2026 contra el código y los tests de `codex/pin-padding-and-gap-table`.
...
| B — Trusted Lists (TS 119 612) | **Hecha**: `TrustedListInspector`, sin `dss-tsl-validation`. El modelo JAXB del spec ya venía por `dss-validation`, y el módulo que falta es el que descarga y refresca por red, que es justo lo que no queremos. TS 119 602 (modelo JSON) está cubierto por `TrustedEntityListJsonInspector` y `TrustedEntityListJsonInspectorTest`. |
...
| Carril pagos — Thales Key Block | **Hecha para 3DES y AES**: `ThalesKeyBlockOperations` lee, construye, cifra, descifra y autentica ambas variantes. La derivación AES y los bloques completos están fijados por capturas en `ThalesKeyBlockAesTest`; la variante 3DES está cubierta por `ThalesKeyBlockOperationsTest` y `ThalesKeyBlockCryptoTest`. |
| Carril pagos — resto | Pendiente: cifrado y MAC del AKB de Atalla (la cabecera sí está), comandos host aún sin cuerpo verificado, ICC Dynamic Number, CAP, AMEX CSC y algunas utilidades. ISO 8583, secure messaging, MAC ISO 9797-1 y gran parte de los PIN blocks heredados y HCE ya tienen implementación y tests; véase la tabla de huecos. FPE se hizo por otra vía. |
...
|---|---|---|
| 1 | **Formatos de protección de fabricante**: Thales LMK y Key Block, Atalla AKB, Futurex MFK y SafeNet KM | **Parcial**: `ThalesLmkOperations` (`ThalesLmkOperationsTest`, `ThalesLmkExternalVectorTest`) y `ThalesKeyBlockOperations` (`ThalesKeyBlockOperationsTest`, `ThalesKeyBlockAesTest`) cubren LMK y Key Block, incluidas variantes 3DES y AES verificadas. `FuturexMfkOperations` (`FuturexMfkOperationsTest`, modificadores 5 y 6) y `SafeNetKmOperations` (`SafeNetKmOperationsTest`, formato 10) están verificados contra capturas. De Atalla, `AtallaAkbHeader` y `AtallaAkbOperations` leen la cabecera AKB; el cifrado y el MAC siguen aparcados. |
| 2 | **EMV ODA**: SDA/DDA/CDA, certificados y firmas de tarjeta | **Cubierto** por `EmvOdaOperations` (`EmvOdaOperationsTest`). `EMVOperations` cubre otras operaciones EMV, no ODA. |
| 3 | **Secure messaging EMV** de Visa y Mastercard | **Cubierto** por `EmvSecureMessaging` (`EmvSecureMessagingTest`) y los nodos `EMV_SM_*` (`PaymentOperationsNodeHandlerTest`). |
| 4 | **ISO 8583**: bitmap y parser de mensajes; ATM NDC, Wincor, AS2805, APACS30 | **Parcial**: `Iso8583Operations` en `crypto/iso8583` cubre construcción, bitmap y parser (`Iso8583OperationsTest`). Los protocolos ATM y perfiles nacionales citados no quedan acreditados por ese test. |
| 5 | **MAC ISO 9797-1 algoritmos 2, 4 y 6** | `MACOperations` implementa 2 y 4, con vectores en `MACOperationsIso9797Test`. También hay método y test local del 6; su paridad con capturas de la herramienta externa sigue aparcada. |
| 6 | **PIN blocks heredados**: Docutel, Diebold, Plus, ECI 1-4, Visa 1-4, Europay/Banksys y AS2805 8.1 | **Parcial**: `PinBlockFormat` y `PaymentOperations` cubren los formatos enumerados salvo AS2805 8.1; `LegacyPinBlockFormatTest` comprueba capturas y el relleno configurable en encode. |
| 7 | **Banco de comandos host de HSM**: payShield A0/BU/CA/CC/CI/CW/CY/DC/EC/FA/GC/HC/JA/KA/M0-M6/NC, Atalla, Futurex | **Parcial**: `PayShieldMessageCodec` (`PayShieldMessageCodecTest`) cubre la envoltura y el prefijo TCP; `PayShieldBodyDecomposer` (`PayShieldBodyDecomposerTest`) cubre descomposición declarativa por código/error. El banco registra como `EXTERNAL_REQUEST` las peticiones A0, BU, CW, CY, A6, A8 y NC, y como `THIRD_PARTY_SIMULATOR` las respuestas A1, BV, CX, CZ, A7 y ND parseadas por la consola. `VERIFIED` queda reservado para un payShield real. NO/NP no se registra porque la consola dio error de parseo; CA, CC, M6 y EE no tienen desglose completo de campos en la captura; las respuestas de error 15 y 30 las cubre el catálogo. Los cruces de KCV y CVV coinciden con `PaymentOperations`. Ver `CAPTURAS_CONSOLA_HSM_PAYSHIELD.md`. |
| 8 | **HCE y tokenización**: Visa LUK/MSD/qVSDC, Mastercard CVC3/PIN-CVC3 y DS, ICC Dynamic Number, CAP y AMEX CSC | **Parcial**: `VisaHceOperations` (`VisaHceOperationsTest`) cubre LUK, MSD y qVSDC; `MastercardDataStorage` (`MastercardDataStorageTest`) cubre DSPK, Digest y Summary. CVC3 y PIN-CVC3 permanecen aparcados porque la herramienta externa ignora UN y ATC. Con las capturas del 26-09-2026 (`docs/CAPTURAS_CALCULADORA_HCE.md` §6) se implementan ICC Dynamic Number y CAP Token en `MastercardIccDynamicNumber` (`MastercardIccDynamicNumberTest`); se rechaza CAP cuando IAF no tiene el bit 40 porque falta un vector que confirme el caso sin PSN. AMEX CSC v1 no se puede deducir porque no hay intermedios, y AMEX CSC v2 lanza una excepción en la herramienta externa. |
| 9 | Utilidades: hashes, CRC32, Base94, BCD, decimalización, bit shift, trace parser, check digit AMEX SE, ATR, APDU y tags EMV | **Parcial**: `HashOperations` (`HashOperationsTest`) cubre MD4, Whirlpool, Tiger-192 y CRC32; `BitShifter` (`BitShifterTest`), `TraceHexExtractor` (`TraceHexExtractorTest`), `CheckDigitCalculator` (`CheckDigitCalculatorTest`), `ApduStatus` (`ApduStatusTest`), `EmvTlv` (`EmvTlvTest`), `Base94Codec` (`CodecRegistryTest`), `DataConverter` (`DataConverterTest`) y `AtrParser` (`AtrParserTest`) cubren las piezas respectivas. La decimalización IBM 3624 está en `PaymentOperations` (`PaymentControlValuesTest`). No se acredita aquí paridad de todas las variantes CRC32. |
