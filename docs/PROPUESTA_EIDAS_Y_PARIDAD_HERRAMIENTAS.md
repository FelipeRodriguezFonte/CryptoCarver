...
| 7 | **Banco de comandos host de HSM**: payShield A0/BU/CA/CC/CI/CW/CY/DC/EC/FA/GC/HC/JA/KA/M0-M6/NC, Atalla, Futurex | **Parcial**: `PayShieldMessageCodec` (`PayShieldMessageCodecTest`) cubre la envoltura y el prefijo TCP; `PayShieldBodyDecomposer` (`PayShieldBodyDecomposerTest`) cubre descomposición declarativa por código/error. `NC` sigue como `PENDING_CAPTURE (NC-00)` y los demás cuerpos no están verificados. La consola HSM externa no trae emulador. El 26-09-2026 se capturaron contra un simulador de terceros las peticiones que arma la consola (NC, NO, A0, A6, A8, BU, CA, CC, CW, CY, M6) y cómo parsea las respuestas. Las respuestas no son de un payShield real; sus CVV y KCV cuadran con `PaymentOperations`. Ver `CAPTURAS_CONSOLA_HSM_PAYSHIELD.md`. |
